...
public:
	...
	void propagate(std::vector<SurfacePoint>& sources,
   				   double max_propagation_distance = INFINITY,			//propagation algorithm stops after reaching the certain distance from the source
...
private:
	...
	void clear()
	{
		...
		m_propagation_distance_stopped = INFINITY;
	};
...
{
	if(p->min() < INFINITY/10.0)// && p->min >= queue->begin()->first)
	{
									   ...
									   std::vector<SurfacePoint>* stop_points)
{
	...
	while(!m_queue.empty())
	{
		...
		assert(min_interval->d() < INFINITY);
	...
	m_propagation_distance_stopped = m_queue.empty() ? INFINITY : (*m_queue.begin())->min();
	clock_t stop = clock();
															 ...
															 unsigned& best_source_index)
{
	...
	if(point.type() == EDGE)		
	{
		...
		if(best_interval)
		{
			//assert(best_interval && best_interval->d() < INFINITY);
...
{					
	...
	if(best_total_distance >= INFINITY/2.0)		//unable to find the right path
	{
		return;
	}
	...
	if(best_interval)	//if we did not hit the face source immediately  
	{
		...
		while(visible_from_source(path.back()) < 0)		//while this point is not in the direct visibility of some source (if we are inside the FACE, we obviously hit the source)
		{
			...
			assert(total_distance<INFINITY);
