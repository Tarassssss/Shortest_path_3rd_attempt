...
protected:
	...
	node_pointer best_first_node(SurfacePoint& point, double& best_total_distance)
	{
		...
		//assert(best_node);
		//assert(best_total_distance<INFINITY);
												 ...
												 std::vector<SurfacePoint>* stop_points) //or after ensuring that all the stop_points are covered
{
	...
	{
		if(m_nodes[i].distance_from_source() < INFINITY)
		{
	...
	while(!m_queue.empty())					//main cycle
	{
		...
		assert(min_node->distance_from_source() < INFINITY);
		...
		for(unsigned i=0; i<visible_nodes.size(); ++i)		//update all the adgecent vertices
		{
			...
			{
				if(next_node->distance_from_source() < INFINITY)		//remove it from the queue
				{
	...
	m_propagation_distance_stopped = m_queue.empty() ? INFINITY : (*m_queue.begin())->distance_from_source();
														 ...
														 std::vector<SurfacePoint>& path) 
{
	...
	if(total_path_length>INFINITY/2.0)		//unable to find the path
	{
		return;
	}
